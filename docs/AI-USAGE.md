...
### Session 6 — the implementation phase (migrations, application, front end)

The five sessions above cover design and research — the plan. Everything
between that plan and the deployed application at
<https://quorum-rho.vercel.app> happened across many further sessions with
Claude Code, working directly in this repository rather than in a separate
research chat. This entry covers that whole stretch at the same honesty bar
as the sessions above, rather than leaving the log looking like the build
stopped after the design was finished.

**Mine.** Every priority call: what Tier 1 had to include before anything
else was touched (`docs/BUILD-PLAN.md`'s ordering), which of the research
tracks' recommendations to actually take (accepting D-004's embeddings cut,
for instance, but pushing back on a forced-tool-call design R5 suggested in
favour of structured outputs), when to stop and ask rather than let a model
guess (D-023's clearance-naming fix, the redirect-vs-restyle decision for the
old full-page chat route, the production-exposure question behind the
showcase accounts), and the standing rule that a claim about authorisation is
never reported without having run the thing that proves it.

**AI-generated, then reviewed by me.** All 22 migrations, the RLS policies on
every table, `lib/` and `app/` in full, the connector and admin-mode
implementations, the demo world and showcase accounts, and the test suite
(628 assertions). Generated by Claude Code, in an ordinary edit-and-review
loop rather than one long unsupervised run.

**How it was checked — the same four commitments held from session one, plus
what running the actual application added:**

1. Every RLS policy was read before being committed — not skimmed for shape,
   read for what it actually permits. `scripts/check-boundaries.mjs` catches
   the mechanical version of this (a memory query outside `lib/memory/`, a
   service-role import outside one file); it does not catch a policy that
   reads fine and is wrong, which is why the reading still happens by hand.
2. Every new authorisation-relevant invariant was proven with a **negative
   control** before being trusted: revert the fix, confirm the specific test
   built to catch it fails, restore the fix. This caught real things —
   `join_default_groups()` excluding demo *chats* as join targets but not
   demo *profiles* as new members (migration 0022) was found by querying
   `chat_members` after creating three showcase accounts and seeing six rows
   that shouldn't exist, not by reading the trigger in advance.
3. `pnpm check` (boundaries, lint, the full test suite against a real
   Postgres) ran before every commit described in this log, not just at the
   end. A regression is worth catching the session it's introduced, not the
   session someone happens to run the suite again.
4. **A UI change was not reported as working without being run.** Most
   verification in this environment used a live authenticated session over
   `curl` — signing in as a seeded account, following the redirect chain,
   confirming the response — rather than a browser, because no Chrome
   install was available here (Playwright's tooling requires the `chrome`
   channel specifically, and installing it failed from insufficient
   permissions in this environment). Where that limitation meant a claim
   could not be verified end to end — a full interactive click-through, a
   captured screenshot — it was stated as unverified rather than assumed.

**Not delegated.** Same as every prior session: the "why" behind a decision.
A model can generate a migration that implements "audience is a learn-time
snapshot"; it did not decide that memory should work that way, or that the
clearance floor needed to be a monotone integer ladder rather than named
teams, or that a knowledge graph should be cut. Those calls, and the argument
for each, are in [`docs/DECISIONS.md`](DECISIONS.md) and were made before any
code implementing them was written.
